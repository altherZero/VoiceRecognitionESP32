...
int cmdTrain(int len, int paraNum)
{
  ...
  printSeperator();
  Serial.println(F("Training... Please speak when prompted."));
  ret = myVR.train(records, paraNum-1, buf);
  
  if (ret >= 0) {
    printTrain(buf, ret);
  } else if (ret == -1) {
    Serial.println(F("Train failed - format error."));
  } else if (ret == -2) {
    Serial.println(F("Train Timeout - speak louder or closer."));
  }
  printSeperator();
  return 0;
}
...
int cmdSigTrain(int len, int paraNum)
{
  ...
  Serial.println(F(" with signature..."));
...
void printHelp(void)
{
  ...
  printSeperator();
  Serial.println(F("train <r0> <r1>...      Train records (max 7)"));
  ...
  Serial.println(F("sigtrain <r> <sig>      Train record with signature"));
  Serial.println(F("settings                Check system settings"));
  Serial.println(F("help                    Show this help"));
  printSeperator();
  Serial.println(F("EXAMPLES:"));
  Serial.println(F("  train 0 1 2          Train records 0, 1, and 2"));
  ...
  Serial.println(F("  sigtrain 0 LIGHT     Train record 0 with signature 'LIGHT'"));
}