...
// Constructor for ESP32
VR::VR(HardwareSerial* serialPort, int rxPin, int txPin) {
  _serial = serialPort;
  _rxPin = rxPin;
  _txPin = txPin;
...
void VR::begin(unsigned long baudRate) {
  _baudRate = baudRate;
  
  // Configure serial port with custom pins if provided
  if (_serial == &Serial2) {
...
int VR::checkSignature(uint8_t record, uint8_t *buf) {
  ...
  send_pkt(FRAME_CMD_CHECK_SIG, record, NULL, 0);
  ret = receive_pkt(vr_buf);
...
int VR::clear() {
  int len;
  send_pkt(FRAME_CMD_CLEAR, NULL, 0);
...
int VR::checkRecognizer(uint8_t *buf) {
  int len;
  send_pkt(FRAME_CMD_CHECK_BSR, NULL, 0);
  len = receive_pkt(vr_buf);
...
int VR::checkRecord(uint8_t *buf, uint8_t *records, uint8_t len) {
  ...
  if (records == NULL && len == 0) {
    memset(buf, 0xF0, 255);
    send_pkt(FRAME_CMD_CHECK_TRAIN, 0xFF, NULL, 0);
    start_millis = millis();
...
/**
    @brief check user group content.
    @param grp --> user group number.
           buf --> return value
             buf[8i]    -->  group number.
...
int VR::checkUserGroup(uint8_t grp, uint8_t *buf) {
  ...
  if (grp == GROUP_ALL) {
    send_pkt(FRAME_CMD_GROUP, FRAME_CMD_GROUP_CUGRP, NULL, 0);
    start_millis = millis();
...
/**
    @brief load system group content to recognizer.
    @param grp --> system group number.
           buf  -->  return value.
             buf[0]     -->  Number of valid voice records in recognizer.
             buf[i+1]   -->  Record number.(0xFF: Not loaded(Nongroup mode), or not set (Group mode)) 
                (i= 0, 1, ... 6)
...
/**
    @brief load user group content to recognizer.
    @param grp --> user group number.
           buf  -->  return value.
             buf[0]     -->  Number of valid voice records in recognizer.
             buf[i+1]   -->  Record number.(0xFF: Not loaded(Nongroup mode), or not set (Group mode)) 
...
int VR::restoreSystemSettings() {
  int len;
  send_pkt(FRAME_CMD_RESET_DEFAULT, NULL, 0);
  len = receive_pkt(vr_buf);
...
int VR::checkSystemSettings(uint8_t* buf) {
  ...
  send_pkt(FRAME_CMD_CHECK_SYSTEM, NULL, 0);
  len = receive_pkt(vr_buf);
...
int VR::setBaudRate(unsigned long br) {
  ...
  send_pkt(FRAME_CMD_SET_BR, baud_rate, NULL, 0);
  ret = receive_pkt(vr_buf);
...
int VR::setIOMode(io_mode_t mode) {
  ...
  int ret;
  send_pkt(FRAME_CMD_SET_IOM, mode, NULL, 0);
  ret = receive_pkt(vr_buf);
...
int VR::resetIO(uint8_t *ios, uint8_t len) {
  ...
  if (len == 1 && ios == NULL) {
    send_pkt(FRAME_CMD_RESET_IO, 0xFF, NULL, 0);
  } else if (len != 0 && ios != NULL) {
...
int VR::setPulseWidth(uint8_t level) {
  ...
  send_pkt(FRAME_CMD_SET_PW, level, NULL, 0);
  ret = receive_pkt(vr_buf);
...
/**
    @brief test function.
    @param cmd --> test command (read/write)
           bsr --> buffer for test data
    @retval 0 --> success
            -1 --> failed
            -2 --> timeout
*/
...
/** Utility function: get length of buffer */
...
/** Utility function: compare buffers */
